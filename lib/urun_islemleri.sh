...
# Varsayılan dosya yolunu tanımla
DEPO_DOSYASI="$PWD/depo.csv"
...
urun_ekle() {
    ...
    local urun_no=$(($(tail -n 1 "$DEPO_DOSYASI" 2>/dev/null | cut -d',' -f1) + 1))
    progress_bar "Ürün ekleniyor"
    echo "$urun_no,$urun_adi,$stok_miktari,$birim_fiyat,$kategori" >> "$DEPO_DOSYASI"
...
urun_guncelle() {
    ...
    local urun_listesi=$(awk -F',' '{print $1 "|" $2}' "$DEPO_DOSYASI")
    ...
    local mevcut_bilgiler=$(grep "^$secilen_urun," "$DEPO_DOSYASI")
    ...
    if [ "$yeni_ad" != "$mevcut_ad" ] && grep -q ",$yeni_ad," "$DEPO_DOSYASI"; then
        zenity --error --text="Bu ürün adı zaten mevcut!"
        return 1
    fi
    ...
    cp "$DEPO_DOSYASI" "yedekler/depo_$(date +%Y%m%d_%H%M%S).csv"
    sed -i "s/^$secilen_urun,.*/$secilen_urun,$yeni_ad,$yeni_stok,$yeni_fiyat,$yeni_kategori/" "$DEPO_DOSYASI"
...
urun_sil() {
    ...
    local urun_listesi=$(awk -F',' '{print $1 "|" $2 "|" $3 "|" $4 "|" $5}' "$DEPO_DOSYASI")
