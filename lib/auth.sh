...
login() {
    ...
    while [ $giris_hakki -gt 0 ]; do
        ...
        local kullanici_satiri=$(grep ",[^,]*$kullanici_adi$" kullanici.csv)
        
        if [ -n "$kullanici_satiri" ] && echo "$kullanici_satiri" | grep -q ",$md5_parola," && echo "$kullanici_satiri" | grep -q ",aktif,"; then
...
ilk_kullanici_olustur() {
    ...
    local kullanici_adi=$(echo $kullanici_bilgileri | cut -d',' -f1)
    local ad=$(echo $kullanici_bilgileri | cut -d',' -f2)
    local soyad=$(echo $kullanici_bilgileri | cut -d',' -f3)
    local parola1=$(echo $kullanici_bilgileri | cut -d',' -f4)
    local parola2=$(echo $kullanici_bilgileri | cut -d',' -f5)
    
    # Validasyonlar
    [ -z "$kullanici_adi" ] || [ -z "$ad" ] || [ -z "$soyad" ] || [ -z "$parola1" ] && {
    ...
    [[ "$kullanici_adi" =~ [[:space:]] ]] && {
        zenity --error --text="Kullanıcı adı boşluk içeremez!"; 
        return 1;
    }
    
    [ ${#kullanici_adi} -lt 3 ] && {
        zenity --error --text="Kullanıcı adı en az 3 karakter olmalıdır!"; 
        return 1;
    }
    ...
    echo "1,$ad,$soyad,yonetici,$md5_parola,aktif,$kullanici_adi" >> kullanici.csv
    
    zenity --info --text="İlk yönetici kullanıcısı oluşturuldu!\n\nKullanıcı adı: $kullanici_adi\n\nProgram yeniden başlatılacak, lütfen giriş yapın."
